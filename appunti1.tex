...
Mostro ora un risultato di connessione, per farlo premetto un lemma
...
\begin{mypro}
  Sia $\Omega$ compatto e $\pa{C_n} \subseteq C(\Omega)$ successione
  di Cauchy tale che ogni $C_n$ è compatto. Allora se $C_n \to C$
  nella metrica di Hausorff $C$ è connesso
\end{mypro}
\begin{proof}
  Supponiamo, per assurdo che $A,A'$ siano due componenti connesse
  distinte di $C$ (potrebbero essercene anche altre). Devono essere
  chiuse perché componenti connesse di un chiuso e compatte perché
  contenute in un compatto.

  Sia $\delta = \inf _{x\in A} d_{A'}(x)$ per la proposizione
  precedente $\delta \neq 0$. Siccome $C_n \to C$ deve esistere $\bar
  n$ tale che $\forall n \ge \bar n$ si abbia ($(A\cup A')+B_{\delta/4}$)
  
  NOO funziona solo se ci sono solo due componenti connesse!! Forse
  passando alla coppia che minimizza $\inf$ bla bla? Sicuri che esista
  davvero? (e che l'inf non faccia 0) TODO
\end{proof}
...
\bibliography{Selected_Topics_on_Analysis_in_Metric_Sp}
