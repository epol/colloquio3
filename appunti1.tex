...
\begin{proof}
  ...
  $\gamma \in \Gamma$ ha lunghezza $\alpha_{(x,y)} (\mu + 2
  \varepsilon )$, allora $\len ^d \pa{ \gamma _{|\bra{s,t}}} = \alpha
  _{(x,y)} (t-s)$. Tutte le curve $\gamma$ avranno domino $\bra{0,\mu
    + 2 \varepsilon}$

  Definiamo ora, al variare di $t\in \bra{0,\mu + 2 \varepsilon}$
  \[ G(t) = \set{\gamma (t) | \; \gamma \in \Gamma} \]

  \`E evidente che $G(0) = A$ e $G(\mu + 2 \varepsilon) = C$, vogliamo
  ora dimostrare che $G:\; \bra{0,\mu + 2 \varepsilon} \to \set{ S
    \subseteq M}$ ha immagine in $C_s(M)$, \`e continua e ha lunghezza
  minore di $\mu + 2 \varespsilon$
\end{proof}
